...
#define LOGGING_LEVEL_DECLARATION(level) \
void log_##level(Slice s); \
void log_##level(String s); \
void log_##level##v(va_list args, const char *fmt); \
void log_##level(const char *fmt, ...);

LOGGING_LEVEL_DECLARATION(debug)
LOGGING_LEVEL_DECLARATION(info)
LOGGING_LEVEL_DECLARATION(warn)
LOGGING_LEVEL_DECLARATION(err)
...
static T* alloc_array(size_t num) {
  return (T*)alloc(sizeof(T)*num, alignof(T));
}

template<class T>
static T* realloc(T *prev, size_t prev_num, size_t num) {
  return (T*)realloc((void*)prev, sizeof(T)*prev_num, sizeof(T)*num, alignof(T));
}
...
struct File {
  static bool get_contents(const char *path, Array<u8> *result);
  static bool get_contents(const char *path, String *result);
  static bool get_contents(Path path, String *result);
  static bool get_contents(Path path, Array<u8> *result);
  static bool isdir(Path path);
  static bool isfile(Path path);
  static bool write(FILE *f, const void *data, int len);
  static bool read(FILE *f, void *buf, int n);
  static int open(FILE **f, const char *filename, const char *mode);
  static bool change_dir(Path p);
  static bool was_modified(const char *path, u64 *time);
  static bool cwd(Path *p);
  static FileType filetype(Path path);
  static bool list_files(Path p, Array<Path> *result);
};
...
static void* realloc(void *prev, size_t prev_size, size_t size, size_t align) {
...
int Slice::prev(const char *chars, int i) {
  for (--i;; --i) {
    if (i <= 0)
      return 0;
...
int Slice::next(const char *chars, int length, int i) {
  for (++i;; ++i) {
    if (i >= length)
      return length;
    if (is_utf8_trail(chars[i]))
...
bool Slice::tofloat(const char *chars, int length, double *result) {
  char buf[24];
...
bool Slice::toint(const char *chars, int length, int *result) {
  char buf[24];
  while (chars && length && *chars == ' ')
    ++chars, --length;
...
bool Slice::toint_from_hex(const char *chars, int length, int *result) {
  char buf[24];
  while (chars && length && *chars == ' ')
    ++chars, --length;
...
int Slice::from_visual_offset(const char *chars, int length, int x, int tab_width) {
  int visual = 0;
  int i;
...
int Slice::convert_to_unix_endlines(char *chars, int length) {
  char *out = chars;
...
int Slice::visual_offset(const char *chars, int length, int x, int tab_width) {
  if (!chars)
    return 0;
...
bool Slice::equals(const char *chars, int length, const char *str, int n) {
  return length == n && !memcmp(chars, str, n);
}

Slice Slice::token(const char *chars, int length, int *offset, char c) {
  int i = *offset;
  if (i >= length)
    return {};
...
bool Slice::contains(const char *chars, int length, char c) {
  for (int i = 0; i < length; ++i)
    if (chars[i] == c)
      return true;
  return false;
...
bool Slice::contains(const char *s, char c) {
  for (; *s; ++s)
    if (*s == c)
      return true;
  return false;
...
Slice Slice::token(const char *chars, int length, int *offset, const char *c) {
  int i = *offset;
  if (i >= length)
    return {};
...
bool Slice::find(const char *chars, int length, int offset, Slice s, int *result) {
...
bool Slice::find(const char *chars, int length, Slice s, int *result) {
  return find(chars, length, 0, s, result);
}

bool Slice::find(const char *chars, int length, int offset, char c, int *result) {
...
bool Slice::find(const char *chars, int length, char c, int *result) {
  return find(chars, length, 0, c, result);
}

bool Slice::find_r(const char *chars, int length, Slice &s, int *result) {
...
bool Slice::find_r(const char *chars, int length, char c, int *result) {
...
bool Slice::begins_with(const char *chars, int length, int offset, const char *str, int n) {
  return length - offset >= n && !memcmp(str, chars+offset, n);
}

bool Slice::begins_with(const char *chars, int length, int offset, const char *str) {
  int n = strlen(str);
  return length - offset >= n && !memcmp(str, chars+offset, n);
}

bool Slice::ends_with(const char *chars, int length, const char *str) {
  int n = strlen(str);
  return length >= n && !memcmp(str, chars+length-n, n);
}

Slice Slice::create(const char *s, int len) {
  Slice sl;
  sl.chars = s;
  sl.length = len;
  return sl;
...
Slice Slice::create(const char *s) {
  return Slice::create(s, strlen(s));
}

void Slice::print_hex(Slice s) {
  putchar('\n');
...
void String::convert_to_unix_endlines() {
  length = Slice::convert_to_unix_endlines(chars, length);
}

String String::create(const char *str, int len) {
  if (!len || !str)
    return {};
...
void StringBuffer::resize(int l) {
  if (l > length)
    extend(l - length);
  length = l;
}

void StringBuffer::extend(int l) {
  length += l;
  if (cap <= length) {
...
void StringBuffer::insert(int i, const char *str, int n) {
  if (i == length) {
    append(str, n);
    return;
  }
...
void StringBuffer::insert(int i, char c, int n) {
  if (i == length) {
    append(c, n);
    return;
  }
...
void StringBuffer::insert(int i, char c) {
  if (i == length) {
    append(c);
    return;
  }
...
void StringBuffer::insert(int i, Utf8char c) {
  char buf[4];
  int n = 0;
  buf[n++] = c.code & 0xFF;
...
void StringBuffer::insert(int i, Slice s) {
  insert(i, s.chars, s.length);
}

void StringBuffer::insert(int i, StringBuffer s) {
  insert(i, s.chars, s.length);
}
...
void StringBuffer::remove(int i, int n) {
...
void StringBuffer::append(Utf8char c) {
  (*this) += c;
}

void StringBuffer::append(Slice s) {
  (*this) += s;
}

void StringBuffer::append(StringBuffer s) {
  this->append(TO_STR(s));
}

void StringBuffer::append(char c, int n) {
  extend(n);
  for (int i = length-n; i < length; ++i)
    chars[i] = c;
}

void StringBuffer::append(const char *str, int n) {
  if (!n)
    return;
  extend(n);
...
void StringBuffer::append(const Utf8char *str, int n) {
  if (!n)
    return;
  // TODO: @utf8
  for (int i = 0; i < n; ++i) {
...
void StringBuffer::operator+=(long i) {
  char buf[32];
  char* b = buf + 31;
  int neg = i < 0;
  *b-- = 0;
...
void StringBuffer::operator+=(double d) {
  int i;
  if (d < 0) {
    append('-');
    d *= -1;
...
void StringBuffer::operator+=(Utf8char c) {
  char buf[4];
  int n = 0;
  buf[n++] = c.code & 0xFF;
  if (!(c.code & 0xFF00)) goto done;
...
void StringBuffer::operator+=(char c) {
  extend(1);
  chars[length-1] = c;
}
...
void StringBuffer::operator+=(String s) {
  (*this) += s.slice;
}

void StringBuffer::operator+=(StringBuffer b) {
  (*this) += b.slice;
}

void StringBuffer::operator+=(Slice b) {
  if (!b.length)
    return;
  extend(b.length);
...
void StringBuffer::appendv(const char* fmt, va_list args) {
  for (; *fmt; ++fmt) {
    if (*fmt == '{' && fmt[1] == '}') {
...
void StringBuffer::appendf(const char* fmt, ...) {
  va_list args;
  va_start(args, fmt);
  appendv(fmt, args);
  va_end(args);
...
StringBuffer StringBuffer::create(int initial_size) {
...
void Path::prepend(Slice dir) {
  string.insert(0, dir);
  string.insert(dir.length, separator);
}

void Path::prepend(Path p) {
  prepend(p.string.slice);
}

void Path::push(const char *str) {
  push(Slice::create(str));
}

void Path::push(Slice file) {
...
bool File::write(FILE *f, const void *data, int len) {
  return fwrite(data, len, 1, f) != 1;
}

bool File::read(FILE *f, void *buf, int n) {
  return fread(buf, n, 1, f) != 1;
}

int File::open(FILE **f, const char *filename, const char *mode) {
  #ifdef OS_WINDOWS
...
bool File::change_dir(Path p) {
  return !chdir(p.string.chars);
}

bool File::was_modified(const char *path, u64 *time) {
  struct stat attr;
  if (stat(path, &attr)) {
...
FileType File::filetype(Path path) {
  struct stat buf;
...
bool File::list_files(Path p, Array<Path> *result) {
  *result = {};
...
bool File::list_files(Path directory, Array<Path> *result) {
  *result = {};
...
bool File::get_contents(const char *path, Array<u8> *result) {
  size_t size;
  *result = {};
  #ifdef OS_WINDOWS
    FILE *f = 0;
...
bool File::get_contents(const char *path, String *result) {
  Array<u8> r;
...
bool File::get_contents(Path path, String *result) {return get_contents(path.string.chars, result);}
bool File::get_contents(Path path, Array<u8> *result) {return get_contents(path.string.chars, result);}

bool File::isdir(Path path) {
  return filetype(path) == FILETYPE_DIR;
}

bool File::isfile(Path path) {
...
void TempAllocator::push(size_t initial_size) {
...
void TempAllocator::pop() {
  pop_allocator();
}

void TempAllocator::pop_and_free() {
  free();
  pop_allocator();
}
