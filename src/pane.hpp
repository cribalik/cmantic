...
void Pane::render_edit() {
  ...
  if (buffer_viewport.w > 0 && numchars_y() > 0) {
    ...
    if (G.search_term.size) {
      ...
      pos = {0, buf_offset.y};
      while (d.find(G.search_buffer.lines[0].slice, false, &pos) && pos.y < buf_y1)
        canvas.fill(Rect{d.to_visual_pos(pos), G.search_buffer.lines[0].length, 1}, G.search_term_background_color.color, *background_color);
    }
